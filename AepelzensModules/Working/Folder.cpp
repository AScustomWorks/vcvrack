...
void Folder::step()
{
  ...
  sym = clamp(params[SYM_PARAM].value + inputs[SYM_INPUT].value/5.0f * params[SYM_ATT_PARAM].value, -1.0f, 1.0f);
  in = (inputs[GATE_INPUT].value/5.0f + sym) * gain;
...
FolderWidget::Folder(Folder *module) : ModuleWidget(module) {
  setPanel(SVG::load(assetPlugin(plugin, "res/Folder.svg")));
