...
void Burst::step()
{
  ...
  timeParam = clamp(params[TIME_PARAM].value + (params[TIME_ATT_PARAM].value * inputs[TIME_INPUT].value / 10.0 * MAX_TIME),0.0, MAX_TIME);
  pulseParam = clamp(params[REP_PARAM].value + (inputs[REP_INPUT].value * params[REP_ATT_PARAM].value /10.0 * MAX_REPS), 0.0, MAX_REPS);
...
BurstWidget::BurstWidget(Burst *module) : ModuleWidget(module) {
  setPanel(SVG::load(assetPlugin(plugin, "res/Burst.svg")));
