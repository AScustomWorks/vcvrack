...
void Erwin::step() {
  ...
  for(int y=0;y<NUM_CHANNELS;y++) {
    ...
    //limit to 4 octaves
    transposeOctave = clamp((int)round(inputs[TRANSPOSE_INPUT].value / 3.0) + (int)round(params[CHANNEL_TRANSPOSE_PARAM + y].value),-4, 4);
    //limit to 1 octave
...
struct ErwinWidget : ModuleWidget {
  ...
  setPanel(SVG::load(assetPlugin(plugin,"res/Erwin.svg")));
